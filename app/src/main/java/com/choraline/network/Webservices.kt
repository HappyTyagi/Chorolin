 ...
 */
class Webservices(private var mContext: Context, private val mApiListener: APIListener, isShowProgressDialog: Boolean?, msg: String) {
    ...
    fun onNoInternetConnection(api: Int)
    {
        ...
        Utility.showMessageDialog(mContext, mContext.resources.getString(R.string.alert_no_internet_connection))
    }

    fun callLoginAPI(email: String, password: String, gcmToken: String, deviceType: String, api: Int)
    {
    ...
    fun callSignupAPI(first_name: String, last_name: String, email: String, conf_email: String, password: String,
                      conf_password: String, choir_name: String, gcmToken: String, deviceType: String, api: Int)
    {
        if(Utility!!.isNetworkAvailable(mContext))
        {
            var call= mApiInterface.doSignup(first_name, last_name, email, conf_email, password, conf_password, choir_name, gcmToken, deviceType);
            call.enqueue(object : Callback<LoginModel> {
                override fun onResponse(call: Call<LoginModel>, response: Response<LoginModel>) {
                    val mObject = response.body()
                    if (mObject != null) {
    ...
    fun callGetProfileAPI(userId: String, gcmToken: String, deviceType: String, api: Int)
    {
        if(Utility!!.isNetworkAvailable(mContext))
        {
            var call= mApiInterface.doGetProfile(userId, gcmToken, deviceType,AppController.appPref.accessToken)
            call.enqueue(object : Callback<LoginModel> {
                override fun onResponse(call: Call<LoginModel>, response: Response<LoginModel>) {
                    val mObject = response.body()
                    if (mObject != null) {
    ...
    fun callUpdateProfileAPI(userId: String, first_name: String, last_name: String, email: String, conf_email: String, oldpassword:String,
                             password1: String, password2: String, address1: String, address2: String, town: String,
                             postcode: String, country: String, telephone: String, optin: String, choir_name: String, api: Int)
    {
        if(Utility!!.isNetworkAvailable(mContext))
        {
            var call= mApiInterface.doUpdateProfile(userId, first_name, last_name, email, conf_email,oldpassword, password1, password2,
                    address1, address2, town, postcode, country, telephone, optin, choir_name,AppController.appPref.accessToken);
            call.enqueue(object : Callback<GenericResponseModel> {
                override fun onResponse(call: Call<GenericResponseModel>, response: Response<GenericResponseModel>) {
                    val mObject = response.body()
                    if (mObject != null) {
    ...
    fun callGetFreeTrailAPI(api: Int)
    {
        if(Utility!!.isNetworkAvailable(mContext))
        {
            ...
            call.enqueue(object : Callback<FreeTrailModel> {
                override fun onResponse(call: Call<FreeTrailModel>, response: Response<FreeTrailModel>) {
                    val mObject = response.body()
                    if (mObject != null) {
                        onResponseSuccess(mObject, api)
                    }
    ...
    fun callGetComposerListAPI(api: Int) {
        if (Utility!!.isNetworkAvailable(mContext)) {
            ...
            call.enqueue(object : Callback<ComposerModel> {
                override fun onResponse(call: Call<ComposerModel>, response: Response<ComposerModel>) {
                    val mObject = response.body()
                    if (mObject != null) {
                        onResponseSuccess(mObject, api)
                    }
    ...
    fun callGetChoralWorksAPI(composerName: String,timeStamp:String, api: Int) {
        if (Utility!!.isNetworkAvailable(mContext)) {
            ...
            call.enqueue(object : Callback<ChoralWorksModel> {
                override fun onResponse(call: Call<ChoralWorksModel>, response: Response<ChoralWorksModel>) {
                    val mObject = response.body()
                    if (mObject != null) {
                        onResponseSuccess(mObject, api)
                    }
    ...
    fun callGetSongDetailAPI(songId: String, api: Int) {
        if (Utility!!.isNetworkAvailable(mContext)) {
            ...
            call.enqueue(object : Callback<SongDetailModel> {
                override fun onResponse(call: Call<SongDetailModel>, response: Response<SongDetailModel>) {
                    val mObject = response.body()
                    if (mObject != null) {
                        onResponseSuccess(mObject, api)
                    }
    ...
    fun callAddToBasketAPI(songId: String, currencyId: String, userId: String, api: Int) {
        if (Utility!!.isNetworkAvailable(mContext)) {
            var call = mApiInterface.addToBasket(songId, currencyId, userId,AppController.appPref.accessToken)
    ...
    fun callFreeGift(songId: String, currencyId: String, userId: String, api: Int) {
        if (Utility!!.isNetworkAvailable(mContext)) {
            var call = mApiInterface.freeGift(songId, currencyId, userId,"","0",AppController.appPref.accessToken)
    ...
    fun callCheckFreeGift(songId: String, currencyId: String, userId: String, api: Int) {
        if (Utility!!.isNetworkAvailable(mContext)) {
            var call = mApiInterface.checkfreeGift(userId, songId, "abc",AppController.appPref.accessToken)
            call.enqueue(object : Callback<BaseModel> {
                override fun onResponse(call: Call<BaseModel>, response: Response<BaseModel>) {
                    val mObject = response.body()
                    if (mObject != null) {
    ...
    fun callDeleteBasketItemAPI(id: String, currencyId: String, userId: String, api: Int) {
        if (Utility!!.isNetworkAvailable(mContext)) {
            var call = mApiInterface.deleteFromBasket(id, currencyId, userId,AppController.appPref.accessToken)
            call.enqueue(object : Callback<BasketModel> {
                override fun onResponse(call: Call<BasketModel>, response: Response<BasketModel>) {
                    val mObject = response.body()
                    if (mObject != null) {
    ...
    fun callApplyDiscountcodeAPI(discountCode: String, currencyId: String, userId: String, api: Int) {
        if (Utility!!.isNetworkAvailable(mContext)) {
            var call = mApiInterface.applyDiscountCode(discountCode, currencyId, userId,AppController.appPref.accessToken)
            call.enqueue(object : Callback<BasketModel> {
                override fun onResponse(call: Call<BasketModel>, response: Response<BasketModel>) {
                    val mObject = response.body()
                    if (mObject != null) {
    ...
    {
        if (Utility!!.isNetworkAvailable(mContext)) {
            var call = mApiInterface.removeDiscountCode( currencyId,userId,AppController.appPref.accessToken)
            call.enqueue(object : Callback<BasketModel> {
                override fun onResponse(call: Call<BasketModel>, response: Response<BasketModel>) {
                    val mObject = response.body()
                    if (mObject != null) {
    ...
    fun callPurchaseAPI(userId: String, discountCode: String, subTotal: String, currencyId: String,api: Int) {
        if (Utility!!.isNetworkAvailable(mContext)) {
            var call = mApiInterface.purchase(userId, discountCode, subTotal, currencyId,AppController.appPref.accessToken)
            call.enqueue(object : Callback<PurchaseDataModel> {
                override fun onResponse(call: Call<PurchaseDataModel>, response: Response<PurchaseDataModel>) {
                    val mObject = response.body()
                    if (mObject != null) {
                        onResponseSuccess(mObject, api)
                    }
    ...
    fun callSubmitOrderAPI(userId: String, orderId: String, transId: String, api: Int,nonce:String) {
        if (Utility!!.isNetworkAvailable(mContext)) {
            var call = mApiInterface.submitOrder(userId, orderId, transId,AppController.appPref.accessToken,nonce)
            call.enqueue(object : Callback<SubmitOrderModel> {
                override fun onResponse(call: Call<SubmitOrderModel>, response: Response<SubmitOrderModel>) {
                    val mObject = response.body()
                    if (mObject != null) {
                        onResponseSuccess(mObject, api)
                    }
    ...
    fun callGetOrderHistoryAPI(userId: String, api: Int) {
        if (Utility!!.isNetworkAvailable(mContext)) {
            var call = mApiInterface.getOrderHistory(userId,AppController.appPref.historyListTimeStamp,AppController.appPref.accessToken)
    ...
    fun callPurchasedMusicAPI(userId: String, api: Int) {
        if (Utility!!.isNetworkAvailable(mContext)) {
            var call = mApiInterface.getPurchasedMusic(userId,"0",AppController.appPref.accessToken)
            call.enqueue(object : Callback<PurchasedMusicModel> {
                override fun onResponse(call: Call<PurchasedMusicModel>, response: Response<PurchasedMusicModel>) {
                    val mObject = response.body()
                    if (mObject != null) {
                        onResponseSuccess(mObject, api)
                    }
    ...
    fun callContactUsAPI(name: String, email: String, contact_number: String, message: String, api: Int)
    {
        if(Utility!!.isNetworkAvailable(mContext))
        {
